...
import { INCIDENT_TYPE_CHOICES } from "@/lib/incidentTypes";
...
function titleCaseToken(token: string) {
  ...
  if (token === token.toUpperCase() && token.length <= 4 && isAllLetters(token))
    return token;
...
export function getIncidentStyle(description?: string): IncidentStyle {
  ...
  if (d.includes("BREAK") && d.includes("ENTER"))
    return { category: "Break & Enter", color: "#EF4444" };
  ...
  if (
    ...
    return { category: "Impaired/Checks", color: "#14B8A6" };
  if (d.includes("PROPERTY") || d.includes("MISCHIEF") || d.includes("DAMAGE"))
    return { category: "Property", color: "#F472B6" };

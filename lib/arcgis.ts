...
async function tryQueryGeoJSON(
  ...
  const exceededRaw = (props?.exceededTransferLimit ??
    data.exceededTransferLimit) as unknown;
...
export async function fetchIncidentsGeoJSON(input: {
  ...
  const client = input.baseUrl
    ? new ArcGISFeatureLayerClient(input.baseUrl)
    : arcgisCrimeLayer;

  const merged: IncidentFeatureCollection = {
    type: "FeatureCollection",
    features: [],
  };
  let offset = 0;
  ...
  while (exceeded) {
    ...
    const result =
      page ?? (await queryEsriJson({ ...common, offset, count: pageSize }));
